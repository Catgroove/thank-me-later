...
foundation resume uses to decide which local work can
...
Branch mode produces one. The Pipeline commits and pushes from the checkout the work already
lives in. The PR opens with the Ship branch as its head and the repo's default branch as its
base.
